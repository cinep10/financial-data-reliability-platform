...
def is_pageview_collector(row: dict, kv: dict, pv_mode: str) -> bool:
...
def infer_event_name_from_collector(row: dict, kv: dict, pv_mode: str) -> str:
    ...
    if is_pageview_collector(row, kv, pv_mode):
        return "page_view"

    return "other"


def infer_event_name_batch(row: dict, pv_mode: str) -> str:
    event_name = (row.get("event_name") or "").strip().lower()
    if event_name:
        return event_name
    kv = parse_kv(row.get("kv_raw"))
    return infer_event_name_from_collector(row, kv, pv_mode)


def is_page_event_batch(row: dict, pv_mode: str) -> bool:
    page_type = (row.get("page_type") or "").strip()
    event_name = (row.get("event_name") or "").strip().lower()
    evt = (row.get("evt") or "").strip().lower()
    path = (row.get("path") or "").lower()

    if STATIC_EXT_RE.search(path):
        return False
    if event_name in ("page_view", "loan_view", "card_apply_start", "card_apply_submit", "loan_apply_start", "loan_apply_submit", "auth_attempt", "auth_success", "auth_fail", "otp_request", "risk_login", "login_success"):
        return True
    if pv_mode == "view_only":
        return evt in ("", "view")
...
def truncate_targets(cur, profile_id, dt_from, dt_to, write_legacy: bool):
    cur.execute("DELETE FROM metric_value_hh WHERE profile_id=%s AND dt BETWEEN %s AND %s", (profile_id, dt_from, dt_to))
    cur.execute("DELETE FROM metric_value_day WHERE profile_id=%s AND dt BETWEEN %s AND %s", (profile_id, dt_from, dt_to))
    if write_legacy:
        cur.execute("DELETE FROM stg_ds_metric_hh WHERE profile_id=%s AND dt BETWEEN %s AND %s", (profile_id, dt_from, dt_to))
        cur.execute("DELETE FROM stg_ds_metric_hh_wide WHERE profile_id=%s AND dt BETWEEN %s AND %s", (profile_id, dt_from, dt_to))


def fetch_batch_rows(cur, dt_from, dt_to):
    cur.execute(
        """
        SELECT
            eb.raw_event_id, eb.dt, eb.ts, eb.event_name, eb.service_domain, eb.funnel_stage, eb.is_conversion,
            eb.uid, eb.pcid, eb.sid, eb.device_type, eb.page_type, eb.status, eb.latency_ms,
            er.ip, er.method, er.path, er.query, er.url_norm, er.kv_raw, er.evt
        FROM stg_event_batch eb
        LEFT JOIN event_log_raw er
          ON eb.raw_event_id = er.raw_event_id
        WHERE eb.dt >= %s AND eb.dt < %s
...
def fetch_hour_counts(cur, table_name, dt_from, dt_to, extra_where=""):
    cur.execute(
        f"""
        SELECT dt, HOUR(ts) AS hh, COUNT(*) AS cnt
        FROM {table_name}
        WHERE dt >= %s AND dt < %s {extra_where}
        GROUP BY dt, HOUR(ts)
        """,
        (dt_from, dt_to),
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyzer B v5 for stg_event_batch -> metric tables / legacy hh tables")
    ...
    try:
        with conn.cursor() as cur:
            ...
            rows = fetch_batch_rows(cur, query_start_dt, query_end_dt)
            raw_hour_counts = fetch_hour_counts(cur, "event_log_raw", query_start_dt, query_end_dt)
            collector_hour_counts = fetch_hour_counts(cur, "stg_wc_log_hit", query_start_dt, query_end_dt)
            batch_hour_counts = fetch_hour_counts(cur, "stg_event_batch", query_start_dt, query_end_dt, "AND COALESCE(load_status, 'success') = 'success'")
            ...
            for target_dt in daterange(dt_from, dt_to):
                ...
                for hist_dt in daterange(query_start_dt, target_dt - timedelta(days=1)):
                    for row in rows_by_dt.get(hist_dt, []):
                        kv = parse_kv(row.get("kv_raw"))
                        historical_users.add(pick_identity(row, kv, args.identity_mode))
                ...
                for row in target_rows:
                    ...
                    ts = row["ts"]
                    hh = ts.hour
                    event_name = infer_event_name_batch(row, args.pv_mode)

                    if is_page_event_batch(row, args.pv_mode):
                        pv_count[hh] += 1
                ...
                hh_candidates = set(raw_hour_counts.keys()) | set(collector_hour_counts.keys()) | set(batch_hour_counts.keys())
                hh_list = sorted({
                    hh for (d, hh) in hh_candidates if d == target_dt
                } | set(pv_count.keys()) | set(auth_attempt.keys()) | set(auth_success.keys()) | set(auth_fail.keys()) | set(otp_request.keys()) | set(risk_login.keys()) | set(loan_view.keys()) | set(loan_start.keys()) | set(loan_submit.keys()) | set(card_start.keys()) | set(card_submit.keys()))

                note = (
                    f"identity={args.identity_mode}; pv_mode={args.pv_mode}; "
                    f"source=stg_event_batch; raw_compare=event_log_raw; collector_compare=stg_wc_log_hit; "
                ...
                for hh in hh_list:
                    ...
                    raw_cnt = raw_hour_counts.get((target_dt, hh), 0)
                    collector_cnt = collector_hour_counts.get((target_dt, hh), 0)
                    batch_cnt = batch_hour_counts.get((target_dt, hh), 0)
                    ...
                    metrics = [
                        ("daily_active_users", dau, dau, None),
                        ("page_view_count", pageviews, pageviews, None),
                        ("avg_session_duration_sec", avg_sess, avg_sess, None),
                        ("new_user_ratio", new_ratio, len(new_users), len(target_users)),
                        ("login_success_count", login_success.get(hh, 0), login_success.get(hh, 0), None),
                        ("auth_attempt_count", attempt, attempt, None),
                        ("auth_success_count", success, success, None),
                        ("auth_fail_count", fail, fail, None),
                        ("auth_success_rate", succ_rate, success, attempt),
                        ("auth_fail_rate", fail_rate, fail, attempt),
                        ("otp_request_count", otp, otp, None),
                        ("risk_login_count", risk, risk, None),
                        ("loan_view_count", loan_v, loan_v, None),
                        ("loan_apply_start_count", loan_s, loan_s, None),
                        ("loan_apply_submit_count", loan_sub, loan_sub, None),
                        ("card_apply_start_count", card_s, card_s, None),
                        ("card_apply_submit_count", card_sub, card_sub, None),
                        ("card_apply_submit_rate", card_submit_rate, card_sub, card_s),
                        ("raw_event_count", raw_cnt, raw_cnt, None),
                        ("collector_event_count", collector_cnt, collector_cnt, None),
                        ("batch_event_count", batch_cnt, batch_cnt, None),
                        ("estimated_missing_rate", missing_rate, max(raw_cnt - batch_cnt, 0), raw_cnt),
                    ]
                ...
                raw_day = sum(v for (d, _), v in raw_hour_counts.items() if d == target_dt)
                collector_day = sum(v for (d, _), v in collector_hour_counts.items() if d == target_dt)
                batch_day = sum(v for (d, _), v in batch_hour_counts.items() if d == target_dt)
                missing_day = round(max(raw_day - batch_day, 0) / raw_day, 6) if raw_day else 0.0
                avg_sess_day = round(sum(sum(v) for v in session_duration_values.values()) / max(sum(len(v) for v in session_duration_values.values()), 1), 6)
                ...
                daily_metrics = [
                    ...
                    ("batch_event_count", batch_day, batch_day, None),
                    ("estimated_missing_rate", missing_day, max(raw_day - batch_day, 0), raw_day),
        ...
        conn.commit()
        print(
            f"[analyzer_b_v5] source=stg_event_batch compare_raw=event_log_raw compare_collector=stg_wc_log_hit "
            f"profile_id={args.profile_id} dt_from={dt_from} dt_to={dt_to} processed_days={processed_days} rows={total_target_rows}"
        )
    finally:
        conn.close()
