...
MODEL_VERSION = "stream_multiclass_model_v2"
...
def main():
    ...
    rows = list(
        zip(
            ...
            pd.Series(preds).astype(str).tolist(),
            [None if p is None else float(p) for p in max_probs],
            [MODEL_VERSION] * len(df),
        )
    )

    print("[DEBUG] sample rows =", rows[:3])
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.executemany("""
                INSERT INTO stream_ml_prediction_day
                (profile_id, dt, service_domain, predicted_label, anomaly_prob, model_version)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, rows)

        conn.commit()
        print(f"[predict_stream_multiclass_to_db_v1] done rows={len(rows)}")
    finally:
        conn.close()
