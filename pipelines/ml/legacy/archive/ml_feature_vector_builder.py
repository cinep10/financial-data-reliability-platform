...
def scenario_label(scenario_name, intensity, score, active):
    ...
    # baseline anchor: scenario가 없으면 normal
    if active == 0 or scenario_name in ("", "baseline", "none", "null"):
        return 0, "normal", "baseline_default"

    # campaign_spike는 business event 성격이므로 완화
    if scenario_name == "campaign_spike":
        if intensity == "severe" and score >= 0.65:
            return 1, "warning", "campaign_adjusted"
        return 0, "normal", "campaign_adjusted"

    # incident 계열
    if scenario_name in ("funnel_break", "auth_failure", "weather_drop", "mixed_incident"):
        if intensity == "light":
            if score >= 0.70:
                return 2, "alert", "scenario_light_override"
            return 1, "warning", "scenario_light_override"

        if intensity == "medium":
            if score >= 0.80:
                return 2, "alert", "scenario_medium_override"
            return 1, "warning", "scenario_medium_override"

        # severe
        if score >= 0.55:
            return 2, "alert", "scenario_severe_override"
        return 1, "warning", "scenario_severe_override"

    # unknown scenario
    if score >= 0.75:
        return 2, "alert", "unknown_scenario_score"
    if score >= 0.45:
        return 1, "warning", "unknown_scenario_score"
...
with conn:
    with conn.cursor() as cur:
        ...
        if not avg_col:
            raise SystemExit("avg session duration column not found in ml_feature_vector_day")
        if not risk_score_col or not risk_grade_col:
            raise SystemExit("risk score / risk grade columns not found in data_risk_score_day_v3")
        ...
        cur.execute(
            ...
            LEFT JOIN (
              ...
              ON m.profile_id=v.profile_id AND m.dt=v.dt
            LEFT JOIN (
              ...
              ON m.profile_id=ds.profile_id AND m.dt=ds.dt
            LEFT JOIN (
              SELECT profile_id, dt,
                     SUM(CASE WHEN anomaly_status='alert' THEN 1 ELSE 0 END) AS alert_cnt,
                     SUM(CASE WHEN anomaly_status='warn' THEN 1 ELSE 0 END) AS warn_cnt
              FROM metric_time_anomaly_day
              GROUP BY profile_id, dt
            ) ta
              ON m.profile_id=ta.profile_id AND m.dt=ta.dt
            LEFT JOIN (
              SELECT profile_id, dt,
                     SUM(CASE WHEN anomaly_status='alert' THEN 1 ELSE 0 END) AS alert_cnt,
                     SUM(CASE WHEN anomaly_status='warn' THEN 1 ELSE 0 END) AS warn_cnt
              ...
              ON m.profile_id=ca.profile_id AND m.dt=ca.dt
        ...
        cur.execute(
            ...
            SELECT
              ...
              CASE
                WHEN SUM(CASE WHEN {risk_grade_col}='high' THEN 1 ELSE 0 END) > 0 THEN 'alert'
                WHEN SUM(CASE WHEN {risk_grade_col}='medium' THEN 1 ELSE 0 END) > 0 THEN 'warning'
                ELSE 'normal'
              END AS agg_risk_status
        ...
        insert_cols = [
            "profile_id", "dt", "daily_active_users", "page_view_count", avg_col, "new_user_ratio",
            "auth_attempt_count", "auth_success_count", "auth_fail_count", "auth_success_rate",
            "auth_fail_rate", "otp_request_count", "risk_login_count", "loan_view_count",
            "loan_apply_start_count", "loan_apply_submit_count", "card_apply_start_count",
            "card_apply_submit_count", "card_apply_submit_rate", "collector_event_count",
            "raw_event_count", "estimated_missing_rate", "validation_fail_count",
            "validation_warn_count", "drift_alert_count", "drift_warn_count",
            "ml_feature_alert_count", "ml_feature_warn_count", "target_risk_status",
            "target_risk_score", "run_id", "note"
        ]
        for extra in [
            "target_risk_label",
            "label_source",
        ...
        def find_run(dt):
            for r in runs:
                if str(r["dt_from"]) <= dt <= str(r["dt_to"]):
                    return r
            return None
        ...
        for row in rows:
            ...
            label, status, label_source = scenario_label(
            ...
            vals_map = {
                ...
                "dt": row["dt"],
                "daily_active_users": safe_float(row.get("daily_active_users")),
                "page_view_count": safe_float(row.get("page_view_count")),
                avg_col: safe_float(avgv),
                "new_user_ratio": safe_float(row.get("new_user_ratio")),
                "auth_attempt_count": safe_float(row.get("auth_attempt_count")),
                "auth_success_count": safe_float(row.get("auth_success_count")),
                "auth_fail_count": safe_float(row.get("auth_fail_count")),
                "auth_success_rate": safe_float(row.get("auth_success_rate")),
                "auth_fail_rate": safe_float(row.get("auth_fail_rate")),
                "otp_request_count": safe_float(row.get("otp_request_count")),
                "risk_login_count": safe_float(row.get("risk_login_count")),
                "loan_view_count": safe_float(row.get("loan_view_count")),
                "loan_apply_start_count": safe_float(row.get("loan_apply_start_count")),
                "loan_apply_submit_count": safe_float(row.get("loan_apply_submit_count")),
                "card_apply_start_count": safe_float(row.get("card_apply_start_count")),
                "card_apply_submit_count": safe_float(row.get("card_apply_submit_count")),
                "card_apply_submit_rate": safe_float(row.get("card_apply_submit_rate")),
                "collector_event_count": safe_float(row.get("collector_event_count")),
                "raw_event_count": safe_float(row.get("raw_event_count")),
                "estimated_missing_rate": safe_float(row.get("estimated_missing_rate")),
                "validation_fail_count": safe_int(row.get("validation_fail_count")),
                "validation_warn_count": safe_int(row.get("validation_warn_count")),
                "drift_alert_count": safe_int(row.get("drift_alert_count")),
                "drift_warn_count": safe_int(row.get("drift_warn_count")),
                "ml_feature_alert_count": safe_int(row.get("ml_feature_alert_count")),
                "ml_feature_warn_count": safe_int(row.get("ml_feature_warn_count")),
                "target_risk_status": status,
                "target_risk_score": score,
                "run_id": f"mlfv_{row['profile_id']}_{dt.replace('-', '')}",
                "note": f"scenario_active={active}; scenario={scenario_name}; intensity={intensity}",
                "target_risk_label": label,
                "label_source": label_source,
                "scenario_active_flag": active,
                "scenario_name": scenario_name,
    ...
    conn.commit()
    print(
        f"[OK] ml_feature_vector_builder_v5 completed: "
        f"rows={len(rows)}, avg_col={avg_col}, risk_score_col={risk_score_col}, risk_grade_col={risk_grade_col}"
    )
