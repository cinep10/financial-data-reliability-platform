...
def main():
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT profile_id, dt, predicted_risk_status FROM ml_prediction_result WHERE profile_id=%s AND dt BETWEEN %s AND %s",
                ...
                for feat in feats:
                    ...
                    cur.execute(
                        ...
                        (
                            ...
                            baseline, float(obs), baseline_sd, score, "zscore",
                            drift_status, severity,
                            f"mlfd_{row['profile_id']}_{str(row['dt']).replace('-', '')}",
                            note,
                        ),
                    )

            conn.commit()
            print(f"[OK] ml_feature_drift_analyzer_safe completed: features={len(feats)}")
    finally:
        conn.close()
