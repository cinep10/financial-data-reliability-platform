...
get_conn <- function() {
  ...
  if (backend == "sqlite") {
    ...
    path <- Sys.getenv("SQLITE_PATH")
    if (path == "") stop("SQLITE_PATH is required when DB_BACKEND=sqlite")
...
align_to_table <- function(df, table_meta, run_id_str, run_id_num) {
  ...
  if ("created_at" %in% table_cols && !("created_at" %in% names(df))) {
    df$created_at <- Sys.time()
  }
  if ("updated_at" %in% table_cols && !("updated_at" %in% names(df))) {
  ...
  if ("detail" %in% table_cols && !("detail" %in% names(df))) {
    df$detail <- NA_character_
  }
  if ("note" %in% table_cols && !("note" %in% names(df))) {
    df$note <- NA_character_
  }
  if ("severity" %in% table_cols && !("severity" %in% names(df))) {
    df$severity <- "low"
  }
  ...
  # force integer-ish columns where useful
  if ("hh" %in% names(out)) {
