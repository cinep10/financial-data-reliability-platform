...
do
  ...
  echo "--------------------------------------------"
  echo "[RUN] $START_DATE ~ $END_DATE | $SCENARIO_NAME | $INTENSITY"
  ...
  if [[ "$SCENARIO_NAME" == "baseline" ]]; then
    ...
    TRAIN_ALL_HISTORY=true     bash "$PROJECT_ROOT/deploy/run_ml_backfill_pipeline_v2.sh" "$START_DATE" "$END_DATE" "$PROFILE_ID"
  else
    bash "$PROJECT_ROOT/deploy/run_scenario_test_pipeline.sh"       "$SCENARIO_NAME" "$START_DATE" "$END_DATE" "$PROFILE_ID" "$INTENSITY"
  fi
done
