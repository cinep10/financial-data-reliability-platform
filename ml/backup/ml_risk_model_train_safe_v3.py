...
def coerce_feature_frame(df: pd.DataFrame, feats: list[str]) -> pd.DataFrame:
    X = df[feats].copy()
...
def main():
    ...
    if "target_risk_status" in df.columns:
        ...
        y = pd.cut(score, bins=[-1, 0.4, 0.7, 999999], labels=[0, 1, 2]).astype(int)
        label_definition = "target_risk_score thresholds 0.4 / 0.7"
    ...
    print(f"[OK] ml_risk_model_train_safe completed: rows={len(df)}, features={len(feats)}")
