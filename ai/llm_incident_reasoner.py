...
PROMPT_VERSION = "ai_incident_reasoner_v3"
...
def fallback_reason(context):
    ...
    time_anomaly = context.get("time_anomaly", [])
    corr_anomaly = context.get("correlation_anomaly", [])
    actions = context.get("actions", [])

    incident_level = pred.get("predicted_risk_status") or "warning"
    risk_score = float(risk.get("risk_score") or 0.0)

    top_causes = [str(r.get("cause_type") or "unknown") for r in root_causes[:3]]
    top_drift = [str(r.get("metric_name") or "unknown_metric") for r in drift[:3]]
    top_time = [str(r.get("metric_name") or "unknown_metric") for r in time_anomaly[:2]]
    top_corr = [str(r.get("pair_name") or "unknown_pair") for r in corr_anomaly[:2]]

    title = "Data reliability incident"
    if top_causes:
        title = f"{top_causes[0]} detected"

    executive_summary = (
        f"Risk score is {risk_score:.3f} and predicted status is {incident_level}. "
        f"Primary signals are root causes={top_causes[:2]} and drift metrics={top_drift[:2]}."
    )

    technical_summary = (
        f"Observed root causes: {', '.join(top_causes) if top_causes else 'none'}. "
        f"Top drift metrics: {', '.join(top_drift) if top_drift else 'none'}. "
        f"Time anomalies: {', '.join(top_time) if top_time else 'none'}. "
        f"Correlation anomalies: {', '.join(top_corr) if top_corr else 'none'}."
    )

    if incident_level == "alert":
        business_impact = "High likelihood of business KPI degradation or funnel disruption."
    elif incident_level == "warning":
        business_impact = "Moderate likelihood of conversion or operational impact."
    else:
        business_impact = "Low immediate business impact, but continued monitoring is recommended."

    recommended_actions = []
    for a in actions[:3]:
        fx = a.get("recommended_fix")
        if fx:
            recommended_actions.append(str(fx))
    if not recommended_actions:
        if top_causes:
            recommended_actions.append(f"Review root cause: {top_causes[0]}")
        if top_drift:
            recommended_actions.append(f"Check drifted metric: {top_drift[0]}")
        if top_corr:
            recommended_actions.append(f"Inspect broken metric relationship: {top_corr[0]}")
        recommended_actions.append("Validate upstream collection, mapping, and recent deployment changes")
    ...
    return {
        ...
        "confidence_score": 0.65 if incident_level != "normal" else 0.80,
        "llm_mode": "fallback_rule_based",
    }
        ...
        with conn.cursor() as cur:
            ...
            for dt in dates:
                ...
                try:
                    ...
                    parsed = fallback_reason(context)
                ...
                print(
                    ...
                    f"profile_id={args.profile_id}, dt={dt}, level={parsed['incident_level']}, model={model}"
                )
